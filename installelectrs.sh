echo "Enter futurebit RPC password.  If you have not changed the RPC password, use futurebit"; read -s rpcPassword
cd ~
git clone https://github.com/romanz/electrs 
cd electrs 
sudo apt install -y clang cmake build-essential cargo tor prometheus
echo "This will run for 25+ minutes!!!"
cargo build --locked --release 
mkdir /media/nvme/electrs 
sudo chmod 777 /media/nvme/electrs 
mkdir /var/lib/tor/electrs_hidden_service/ 
sudo echo "

#Electrs
HiddenServiceDir /var/lib/tor/electrs_hidden_service/ 
HiddenServicePort 50001 
127.0.0.1:50001 HiddenServiceVersion 3
" >> /etc/torrc/tor

sudo usermod -a -G debian-tor futurebit
sudo chown debian-tor /var/lib/tor/electrs_hidden_service 
sudo chmod --reference=/var/lib/tor/lnd /var/lib/tor/electrs_hidden_service/ 
